...
@section('title', 'Offline Action - Watery Nation')
        ...
        <div class="row align-items-start mb-5">
            <div class="col-md-12" style="padding: 0 40px;">
                <h3 class="fw-bold mb-4"
                    ...
                    Modul Development For Kids <span class="text-primary">Watery Nation</span>
                    <span
                ...
                <div class="row g-4">
                    @forelse ($modulDevelopmentForKids as $programs)
                        <div class="col-12 col-md-6 col-lg-3">
                            <a href="{{ route('programs.show', $programs->slug) }}"
                                class="text-decoration-none text-dark">
                                <div class="card h-100 shadow-sm rounded-3">
                                    <div class="position-relative border-bottom border-3">
                                        <img src="{{ Storage::url($programs->image) }}"
                                            class="card-img-top rounded-top-3 object-cover"
                                            style="width: 100%; height: 200px;" alt="{{ $programs->title }}">
                                    </div>
                                    <div class="card-body">
                                        <h5 class="card-title fw-bold mb-3">{{ $programs->title }}</h5>
                                        <!-- Kategori kecil -->
                                        <div class="d-flex flex-column gap-2 mb-3" style="font-size:0.9rem;">
                                            <div class="d-flex align-items-center gap-3 text-muted" style="font-size:12px;">
                                                ...
                                                <span>{{ tanggal($programs->start_date) }} |
                                                    {{ tanggal($programs->end_date) }}</span>
                                            </div>
                                            <div class="d-flex align-items-center gap-3 text-muted" style="font-size:12px;">
                                                <i class="bi bi-geo-alt-fill"></i>
                                                <span>{{ $programs->lokasi }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    @empty
                        <div class="col-12">
                            <p class="text-center">Program Modul Development For Kids belum tersedia.</p>
                        </div>
                    @endforelse
                </div>
            </div>
